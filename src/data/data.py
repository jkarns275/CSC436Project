...
class Data:
    ...
    def update_steering_correction(self):
        if self.timer.check_dt() > .125 and self.target_speed > 40:
            ...
            if delta_left > delta_right:
                self.left_steering_correction_avg = self.calculate_left_steering_correction_avg(((float(delta_right) / float(delta_left))) * Constants.STEERING_CORRECTION_POS_COMPENSATION)
                self.right_steering_correction_avg = self.calculate_right_steering_correction_avg(((float(delta_left) / float(delta_right))) * Constants.STEERING_CORRECTION_NEG_COMPENSATION)
            else:
                self.left_steering_correction_avg = self.calculate_left_steering_correction_avg(((float(delta_right) / float(delta_left))) * Constants.STEERING_CORRECTION_NEG_COMPENSATION)
                self.right_steering_correction_avg = self.calculate_right_steering_correction_avg(((float(delta_left) / float(delta_right))) * Constants.STEERING_CORRECTION_POS_COMPENSATION)
    ...
    def calculate_right_steering_correction_avg(self, new_value):
        return 0.125 * new_value + (1.0 - 0.125) * self.right_steering_correction_avg

    def calculate_left_steering_correction_avg(self, new_value):        
        return 0.125 * new_value + (1.0 - 0.125) * self.left_steering_correction_avg
